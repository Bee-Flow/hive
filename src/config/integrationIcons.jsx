...
export const INTEGRATION_ICONS = {
    ...
    linkedin: (
        <svg className="w-5 h-5" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><rect x="2" y="2" width="20" height="20" rx="4" fill="#0A66C2" /><path d="M7.5 9.5h2v7h-2v-7zm1-3.2a1.2 1.2 0 110 2.4 1.2 1.2 0 010-2.4zm3.5 3.2h1.9v1h0c.27-.5 .92-1.1 1.9-1.1 2 0 2.4 1.3 2.4 3.1v3.6h-2v-3.2c0-.8 0-1.8-1.1-1.8s-1.3.9-1.3 1.7v3.3h-2v-6.6z" fill="white" /></svg>
    ),
    github: (
        <svg className="w-5 h-5" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path fillRule="evenodd" clipRule="evenodd" d="M12 2C6.477 2 2 6.484 2 12.017c0 4.425 2.865 8.18 6.839 9.504.5.092.682-.217.682-.483 0-.237-.008-.868-.013-1.703-2.782.605-3.369-1.343-3.369-1.343-.454-1.158-1.11-1.466-1.11-1.466-.908-.62.069-.608.069-.608 1.003.07 1.531 1.032 1.531 1.032.892 1.53 2.341 1.088 2.91.832.092-.647.35-1.088.636-1.338-2.22-.253-4.555-1.113-4.555-4.951 0-1.093.39-1.988 1.029-2.688-.103-.253-.446-1.272.098-2.65 0 0 .84-.27 2.75 1.026A9.564 9.564 0 0112 6.844c.85.004 1.705.115 2.504.337 1.909-1.296 2.747-1.027 2.747-1.027.546 1.379.203 2.398.1 2.651.64.7 1.028 1.595 1.028 2.688 0 3.848-2.339 4.695-4.566 4.943.359.309.678.92.678 1.855 0 1.338-.012 2.419-.012 2.747 0 .268.18.58.688.482A10.02 10.02 0 0022 12.017C22 6.484 17.522 2 12 2z" fill="#181717" /></svg>
    ),
};

const FallbackIcon = (
    <svg className="w-5 h-5" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M9 3h6v4h4v6h-4v4h-6v-4H5v-6h4V3z" stroke="currentColor" strokeWidth="1.5" strokeLinejoin="round" /></svg>
);

export function getIntegrationIcon(id) {
    if (!id) return FallbackIcon;
    // Tool callers spell integration ids in either kebab- or snake_case
    // (`google-calendar` vs `google_calendar`) — normalise both directions
    // before falling back, so a DB row keyed `google_drive` still resolves.
    return (
        INTEGRATION_ICONS[id]
        || INTEGRATION_ICONS[id.replace(/_/g, '-')]
        || INTEGRATION_ICONS[id.replace(/-/g, '_')]
        || FallbackIcon
    );
}

/**
 * True when the registry has a brand icon for this id (after normalisation).
 * Callers use this to fall back to a generic icon container when there's no
 * brand asset rather than rendering the puzzle-piece placeholder.
 */
export function hasIntegrationIcon(id) {
    if (!id) return false;
