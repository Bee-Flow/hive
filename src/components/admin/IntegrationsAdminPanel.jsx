                ...
                <div className="rounded-2xl border overflow-hidden" style={{ background: 'var(--bg-secondary)', borderColor: 'var(--border-default)' }}>
                    <div className="px-6 py-4 border-b" style={{ borderColor: 'var(--border-subtle)' }}>
                        <h3 className="font-semibold flex items-center gap-2" style={{ color: 'var(--text-primary)' }}>
                            <svg className="w-4 h-4" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M12 2L2 7l10 5 10-5-10-5z" fill="#0078D4"/><path d="M2 17l10 5 10-5" stroke="#0078D4" strokeWidth="2" fill="none"/><path d="M2 12l10 5 10-5" stroke="#50A0E0" strokeWidth="2" fill="none"/></svg>
                            Azure AI Speech — Meeting Transcription
                            {hasAzureSpeechKey && <span className="text-xs px-2 py-0.5 rounded-full bg-green-500/10 text-green-500">Configured</span>}
                        </h3>
                        <p className="text-xs mt-0.5" style={{ color: 'var(--text-muted)' }}>
                            Use Azure AI Speech with the Whisper model for high-accuracy transcription with speaker diarization. Speaker names are identified automatically from context.
                        </p>
                    </div>
                    <div className="p-6 space-y-5">
                        ...
                        <div>
                            ...
                            <div className="flex gap-2">
                                {[{ id: 'voxtral', label: 'Voxtral (Mistral)' }, { id: 'azure', label: 'Azure AI Speech' }].map(opt => (
                                    <button
                                        key={opt.id}
                                        onClick={async () => {
                                            if (transcriptionProvider === opt.id) return;
                                            setSavingTranscriptionProvider(true);
                                            try {
                                                const res = await authFetch(`${API_BASE}/ai/config`, {
                                                    method: 'POST',
                                                    headers: { 'Content-Type': 'application/json' },
                                                    body: JSON.stringify({ transcriptionProvider: opt.id }),
                                                });
                                                if (res.ok) {
                                                    setTranscriptionProvider(opt.id);
                                                    setMessage({ type: 'success', text: `Transcription provider set to ${opt.label}` });
                                                }
                                            } catch (e) {
                                                setMessage({ type: 'error', text: 'Failed to save transcription provider' });
                                            }
                                        ...
                                        style={{
                                            background: transcriptionProvider === opt.id ? 'var(--accent-primary)' : 'var(--bg-primary)',
                                            color: transcriptionProvider === opt.id ? '#fff' : 'var(--text-muted)',
                                            border: `1px solid ${transcriptionProvider === opt.id ? 'var(--accent-primary)' : 'var(--border-default)'}`,
                                        }}
                                    >
                                        {opt.label}
                                    </button>
                            ...
                            <p className="text-xs mt-1.5" style={{ color: 'var(--text-muted)' }}>
                                <strong>Voxtral</strong> is the default (requires Mistral API key). <strong>Azure AI Speech</strong> uses the Whisper model with enterprise-grade speaker diarization.
                            </p>
                        </div>

                        {/* Azure credentials — shown always so admin can configure in advance */}
                        <div className="pt-3 border-t" style={{ borderColor: 'var(--border-subtle)' }}>
                            <label className="text-sm font-medium flex items-center gap-2 mb-2" style={{ color: 'var(--text-primary)' }}>
                                Azure Speech Credentials
                                {hasAzureSpeechKey && <span className="text-xs px-2 py-0.5 rounded-full bg-green-500/10 text-green-500">Key saved</span>}
                            </label>
                            <div className="flex gap-2 mb-2">
                                <input
                                    type="text"
                                    ...
                                    placeholder="Region (e.g. westeurope, eastus)"
                                    className="flex-1 px-3 py-2 rounded-lg text-sm border outline-none focus:ring-2 transition-all"
                                    style={{ background: 'var(--bg-primary)', borderColor: 'var(--border-default)', color: 'var(--text-primary)', '--tw-ring-color': 'var(--accent-primary)' }}
                                />
                                <input
                                    ...
                                    placeholder={hasAzureSpeechKey ? '••••••••••••••••' : 'Speech resource API key'}
                                    ...
                                    onClick={async () => {
                                        ...
                                        try {
                                            ...
                                            if (res.ok) {
                                                ...
                                                setAzureSpeechKey('');
                                                setMessage({ type: 'success', text: 'Azure Speech credentials saved' });
                                            } else {
                            ...
                            <p className="text-xs" style={{ color: 'var(--text-muted)' }}>
                                Create an <strong>Azure AI Speech</strong> resource at the{' '}
                                <a href="https://portal.azure.com/#create/Microsoft.CognitiveServicesSpeechServices" target="_blank" rel="noopener noreferrer" className="underline" style={{ color: 'var(--accent-primary)' }}>Azure Portal</a>.
                                Copy the <em>Key 1</em> and the <em>Location / Region</em> (e.g. <code>westeurope</code>).
                                The key is stored encrypted (AES-256-GCM) and is never exposed via the API.
                            </p>
                        </div>
                    </div>
                </div>
