...
import { SEG_TO_SECTION, SECTION_TO_SEG, sectionFromRaw, segmentForSection, parseStudioUrl } from './studioRoutes';
...
describe('seg ↔ section maps', () => {
    ...
    it('accepts every legacy alias from the pre-registry switches', () => {
        ...
        expect(sectionFromRaw('tests')).toBe('tests');
        expect(sectionFromRaw('support')).toBe('support');
        expect(sectionFromRaw('lead-studio')).toBe('leadStudio');
        expect(sectionFromRaw('leadStudio')).toBe('leadStudio');
    ...
    it('maps sections back to the canonical path segments', () => {
        expect(segmentForSection('aiTasks')).toBe('routines');
        expect(segmentForSection('meetingNotes')).toBe('meeting-notes');
        expect(segmentForSection('leadStudio')).toBe('lead-studio');
...
describe('parseStudioUrl', () => {
    ...
    it('treats the reserved routines "steps" segment as routineKind=step', () => {
    ...
    it('parses the remaining studio sections', () => {
        expect(parseStudioUrl('/app/studio/tests').section).toBe('tests');
        expect(parseStudioUrl('/app/studio/support').section).toBe('support');
        expect(parseStudioUrl('/app/studio/lead-studio').section).toBe('leadStudio');
