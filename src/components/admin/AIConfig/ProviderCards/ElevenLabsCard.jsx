...
const ElevenLabsApiKeyCard = ({ onMessage }) => {
    ...
    const fetchKeyStatus = async () => {
        try {
            ...
            if (res.ok) {
                ...
                setHasKey(!!data.hasElevenLabsKey);
            }
        } catch (e) {
            console.error('Failed to fetch ElevenLabs key status:', e);
        }
    };
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                setApiKey('');
                setShowKey(false);
                onMessage?.({ type: 'success', text: 'ElevenLabs API key saved!' });
            } else {
    ...
    const handleDelete = async () => {
        try {
            const res = await authFetch(`${API_BASE}/ai/config/key/elevenlabs_api_key`, { method: 'DELETE' });
            if (res.ok) {
                setHasKey(false);
                setApiKey('');
                setConfirmDelete(false);
                onMessage?.({ type: 'success', text: 'ElevenLabs API key removed' });
            } else {
    ...
    return (
        <div className="mb-6 p-5 rounded-xl border" style={{ background: 'var(--bg-tertiary)', borderColor: 'var(--border-default)' }}>
            <div className="flex items-center gap-3 mb-4">
                <div className="w-10 h-10 rounded-lg flex items-center justify-center text-xl" style={{ background: 'linear-gradient(135deg, rgba(99,102,241,0.2), rgba(236,72,153,0.2))' }}>
                    🎵
                </div>
                <div className="flex-1">
                    <h4 className="font-medium" style={{ color: 'var(--text-primary)' }}>ElevenLabs API Key</h4>
                    <p className="text-xs" style={{ color: 'var(--text-muted)' }}>
                        {hasKey ? '✅ API key configured' : 'Required for Music (with vocals), TTS & Sound Effects'}
                    </p>
                </div>
                {hasKey && (
            ...
            <div className="flex gap-2">
                <div className="relative flex-1">
                    <input
                        ...
                        placeholder={hasKey ? '••••••••••••••••' : 'Enter your ElevenLabs API key'}
            ...
            <p className="text-xs mt-2" style={{ color: 'var(--text-muted)' }}>
                Get your API key from <a href="https://elevenlabs.io/app/settings/api-keys" target="_blank" rel="noopener noreferrer" className="underline hover:text-[var(--accent-primary)]">elevenlabs.io</a>
            </p>
        </div>
    );
};
