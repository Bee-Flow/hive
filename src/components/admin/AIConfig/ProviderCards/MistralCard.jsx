...
const MistralApiKeyCard = ({ onMessage }) => {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                setApiKey('');
                setShowKey(false);
                onMessage?.({ type: 'success', text: 'Mistral API key saved!' });
            } else {
    ...
    const handleDelete = async () => {
        try {
            const res = await authFetch(`${API_BASE}/ai/config/key/mistral_api_key`, { method: 'DELETE' });
            if (res.ok) {
                setHasKey(false);
                setApiKey('');
                setConfirmDelete(false);
                onMessage?.({ type: 'success', text: 'Mistral API key removed' });
            } else {
    ...
    return (
        <div className="mb-6 p-5 rounded-xl border" style={{ background: 'var(--bg-tertiary)', borderColor: 'var(--border-default)' }}>
            <div className="flex items-center gap-3 mb-4">
                <div className="w-10 h-10 rounded-lg flex items-center justify-center text-xl" style={{ background: 'rgba(139, 92, 246, 0.15)' }}>
                    🌪️
                </div>
                <div className="flex-1">
                    <h4 className="font-medium" style={{ color: 'var(--text-primary)' }}>Mistral API Key</h4>
                    <p className="text-xs" style={{ color: 'var(--text-muted)' }}>
                        {hasKey ? '✅ API key configured' : 'Required for Mistral AI models'}
                    </p>
                </div>
                {hasKey && (
            ...
            <div className="flex gap-2">
                <div className="relative flex-1">
                    <input
                        ...
                        placeholder={hasKey ? '••••••••••••••••' : 'Enter your Mistral API key'}
            ...
            <p className="text-xs mt-2" style={{ color: 'var(--text-muted)' }}>
                Get your API key from <a href="https://console.mistral.ai" target="_blank" rel="noopener noreferrer" className="underline hover:text-[var(--accent-primary)]">console.mistral.ai</a>
            </p>
        </div>
    );
};
