...
const ClaudeApiKeyCard = ({ onMessage }) => {
    ...
    const fetchKeyStatus = async () => {
        try {
            ...
            console.error('Failed to fetch Claude key status:', e);
        }
    };
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                setApiKey('');
                setShowKey(false);
                onMessage?.({ type: 'success', text: 'Claude API key saved!' });
            } else {
    ...
    return (
        <div className="mb-6 p-5 rounded-xl border" style={{ background: 'var(--bg-tertiary)', borderColor: 'var(--border-default)' }}>
            <div className="flex items-center gap-3 mb-4">
                <div className="w-10 h-10 rounded-lg flex items-center justify-center text-xl" style={{ background: 'rgba(217, 119, 6, 0.15)' }}>
                    🧠
                </div>
                <div className="flex-1">
                    <h4 className="font-medium" style={{ color: 'var(--text-primary)' }}>Claude API Key</h4>
                    <p className="text-xs" style={{ color: 'var(--text-muted)' }}>
                        {hasKey ? '✅ API key configured' : 'Required for Claude models (Opus, Sonnet, Haiku)'}
                    </p>
                </div>
                {hasKey && (
            ...
            <div className="flex gap-2">
                <div className="relative flex-1">
                    <input
                        ...
                        placeholder={hasKey ? '••••••••••••••••' : 'Enter your Claude API key'}
