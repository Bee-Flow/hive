    ...
    return (
        <div className="flex flex-col h-full min-h-0">
            <div className="flex-1 overflow-y-auto p-6 space-y-6">
                ...
                <Section title={t('ticket_assistant.connection_settings')}>
                    <div className="grid grid-cols-2 gap-3">
                        <Field label={t('ticket_assistant.target_kb')}>
                    ...
                    <div className="grid grid-cols-3 gap-3">
                        <Field label={t('ticket_assistant.sync_interval')}>
                    ...
                    <div className="grid grid-cols-3 gap-3 pt-1">
                        {[
                            ['group_threads', t('ticket_assistant.group_threads')],
                            ['process_attachments', t('ticket_assistant.process_attachments')],
                            ['redact_pii', t('ticket_assistant.redact_pii')],
                        ].map(([key, label]) => (
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        <Field label={t('ticket_assistant.folder_filter')}>
                        ...
                        <Field label={t('ticket_assistant.sender_blacklist')}>
                            <div className="flex gap-1 mb-1.5">
                ...
                <Section title={t('ticket_assistant.ingestion_mode')} description={t('ticket_assistant.ingestion_mode_desc')}>
                    <div className="space-y-2">
                        <label className={`flex items-start gap-2.5 p-3.5 rounded-xl border-2 cursor-pointer transition-all ${effectiveMode === 'per_email' ? 'border-[var(--accent-primary)] bg-[var(--accent-primary)]/5' : 'border-[var(--border-subtle)] hover:border-[var(--border-default)]'}`}>
                            ...
                            <div>
                                <div className="text-[12px] font-semibold text-[var(--text-primary)]">{t('ticket_assistant.ingestion_per_email')}</div>
                                <div className="text-[11px] text-[var(--text-tertiary)] mt-0.5">{t('ticket_assistant.ingestion_per_email_desc')}</div>
                            </div>
                        </label>
                        <label className={`flex items-start gap-2.5 p-3.5 rounded-xl border-2 cursor-pointer transition-all ${effectiveMode === 'category_merge' ? 'border-[var(--accent-primary)] bg-[var(--accent-primary)]/5' : 'border-[var(--border-subtle)] hover:border-[var(--border-default)]'}`}>
                            ...
                            <div>
                                <div className="text-[12px] font-semibold text-[var(--text-primary)]">{t('ticket_assistant.ingestion_category')}</div>
                                <div className="text-[11px] text-[var(--text-tertiary)] mt-0.5">{t('ticket_assistant.ingestion_category_desc')}</div>
                            </div>
                        </label>
                        {effectiveMode === 'per_email' && (
                            <div className="flex items-start gap-2 text-[11px] text-amber-700 p-2.5 bg-amber-50 rounded-lg">
                                ...
                                <span>{t('ticket_assistant.ingestion_per_email_warn')}</span>
                            </div>
                        )}
                    </div>
                </Section>
                ...
                <Section title={t('ticket_assistant.custom_prompt')} description={t('ticket_assistant.custom_prompt_desc')}>
                    {['article', 'category', 'merge'].map(stage => (
                        <details key={stage} className="p-3 rounded-lg bg-[var(--bg-secondary)] border border-[var(--border-subtle)]">
                            <summary className="cursor-pointer text-[12px] font-medium text-[var(--text-primary)] flex items-center justify-between">
                                <span>{t(`ticket_assistant.stage_${stage}`)}</span>
