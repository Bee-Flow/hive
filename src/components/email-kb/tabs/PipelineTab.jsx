...
const STAGES = [
    { key: 'cleanup',    icon: Sparkle,      color: 'slate',   configurable: false, titleKey: 'ticket_assistant.stage_cleanup',    descKey: 'ticket_assistant.stage_cleanup_desc' },
    { key: 'pii',        icon: Shield,       color: 'rose',    configurable: false, titleKey: 'ticket_assistant.stage_pii',        descKey: 'ticket_assistant.stage_pii_desc', togglesSetting: 'redact_pii' },
    { key: 'usefulness', icon: Filter,       color: 'cyan',    configurable: false, titleKey: 'ticket_assistant.stage_usefulness', descKey: 'ticket_assistant.stage_usefulness_desc', comingSoon: true },
    { key: 'article',    icon: Sparkles,     color: 'blue',    configurable: true,  titleKey: 'ticket_assistant.stage_article',    descKey: 'ticket_assistant.stage_article_desc', withParallel: true },
    { key: 'category',   icon: Layers,       color: 'amber',   configurable: true,  titleKey: 'ticket_assistant.stage_category',   descKey: 'ticket_assistant.stage_category_desc' },
    { key: 'merge',      icon: FileText,     color: 'purple',  configurable: true,  titleKey: 'ticket_assistant.stage_merge',      descKey: 'ticket_assistant.stage_merge_desc' },
    { key: 'dedupe',     icon: Combine,      color: 'fuchsia', configurable: true,  titleKey: 'ticket_assistant.stage_dedupe',     descKey: 'ticket_assistant.stage_dedupe_desc', multiChunkOnly: true },
    { key: 'ingest',     icon: CheckCircle2, color: 'emerald', configurable: false, titleKey: 'ticket_assistant.stage_ingest',     descKey: 'ticket_assistant.stage_ingest_desc' },
];
    ...
    return (
        ...
        >
            <div className={`relative w-14 h-14 rounded-2xl flex items-center justify-center border-2 transition-all ${
                ...
                {stage.comingSoon && (
                    <span className="absolute -bottom-1.5 -right-1.5 w-4 h-4 rounded-full bg-[var(--bg-primary)] border border-[var(--border-subtle)] text-[8px] font-bold text-[var(--text-tertiary)] flex items-center justify-center" title={t('ticket_assistant.stage_coming_soon')}>
                        ⋯
                    </span>
                )}
            </div>
            ...
            {tier && stage.configurable && (
                <div className={`mt-1 text-[10px] ${c.text} font-medium`}>{t(`ticket_assistant.tier_${tier}`)}</div>
            )}
            {subLabel && (
            ...
            {inactive && !subLabel && (
                <div className="mt-1 text-[10px] text-[var(--text-tertiary)] italic">
                    {t('ticket_assistant.pipeline_stage_inactive')}
                </div>
            )}
        </button>
    );
        ...
        <div className={`mt-4 p-4 rounded-xl border ${c.border} bg-[var(--bg-primary)] space-y-3`}>
            <div className="flex items-center justify-between gap-3">
                ...
                <div className="flex items-center gap-2">
                    {stage.withParallel && (
                        <>
                            <div className="flex flex-col items-end">
                                <label className="text-[9px] text-[var(--text-tertiary)]" title={t('ticket_assistant.parallel_help')}>{t('ticket_assistant.parallel')}</label>
                            ...
                            <div className="flex flex-col items-end">
                                <label className="text-[9px] text-[var(--text-tertiary)]" title={t('ticket_assistant.batch_help')}>{t('ticket_assistant.batch')}</label>
                    ...
                    <div className="flex flex-col items-end">
                        <label className="text-[9px] text-[var(--text-tertiary)]">{t('ticket_assistant.model_tier')}</label>
                        <select value={cfg.modelTier} onChange={e => update({ modelTier: e.target.value })}
                            ...
                            <option value="fast">{t('ticket_assistant.tier_fast')}</option>
                            <option value="thinking">{t('ticket_assistant.tier_thinking')}</option>
                            <option value="writer">{t('ticket_assistant.tier_writer')}</option>
                            <option value="deep_thinking">{t('ticket_assistant.tier_deep_thinking')}</option>
                        </select>
                    </div>
                </div>
            </div>
            ...
            <details className="text-[11px]">
                <summary className="cursor-pointer text-[var(--text-tertiary)] hover:text-[var(--accent-primary)] inline-flex items-center gap-1">
                    <ChevronRight className="w-3 h-3" /> {t('ticket_assistant.custom_prompt')}
                </summary>
                <textarea value={cfg.systemPrompt}
                    onChange={e => update({ systemPrompt: e.target.value })}
                    placeholder={t('ticket_assistant.custom_prompt_placeholder')} rows={4}
                ...
                {cfg.systemPrompt && (
                    <button onClick={() => update({ systemPrompt: '' })}
                        ...
                        <RotateCcw className="w-3 h-3" /> {t('ticket_assistant.reset_prompt')}
                    </button>
                )}
            </details>
    ...
    return (
        <div className="flex flex-col h-full min-h-0">
            <div className="flex-1 overflow-y-auto p-6 space-y-4">
                <div>
                    <h3 className="text-[13px] font-semibold text-[var(--text-primary)]">{t('ticket_assistant.pipeline_config')}</h3>
                    <p className="text-[11px] text-[var(--text-tertiary)] mt-0.5">{t('ticket_assistant.pipeline_click_configure')}</p>
                </div>

                {isPerEmail && (
                    <div className="text-[11px] text-[var(--text-tertiary)] p-3 bg-[var(--bg-secondary)] border border-[var(--border-subtle)] rounded-lg">
                        {t('ticket_assistant.pipeline_per_email_note')}
                    </div>
                )}
                ...
                <div className="p-4 rounded-xl bg-[var(--bg-secondary)] border border-[var(--border-subtle)] overflow-x-auto">
                    <div className="flex items-start gap-0 min-w-max">
                        {STAGES.map((s, i) => {
                            ...
                            else if (s.comingSoon) subLabel = t('ticket_assistant.stage_coming_soon');
                            else if (s.multiChunkOnly) subLabel = t('ticket_assistant.dedupe_only_multi_chunk');
                            return (
            ...
            {dirty && (
                <div className="flex-shrink-0 flex items-center justify-between gap-3 px-6 py-3 border-t border-[var(--border-subtle)] bg-[var(--bg-primary)] shadow-[0_-4px_12px_rgba(0,0,0,0.04)]">
                    <span className="text-[12px] text-[var(--text-secondary)] flex items-center gap-1.5">
                        ...
                        {t('ticket_assistant.unsaved_changes')}
                    </span>
                    <div className="flex gap-2">
                        <button onClick={discard} disabled={saving}
                            ...
                            {t('ticket_assistant.discard_changes')}
                        </button>
                        <button onClick={save} disabled={saving}
                            ...
                            {saving ? t('ticket_assistant.saving') : t('ticket_assistant.save_changes')}
                        </button>
                    </div>
                </div>
            )}
