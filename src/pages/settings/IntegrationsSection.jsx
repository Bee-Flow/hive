...
const WhatsAppIntegration = ({ onSaved, last }) => {
    ...
    return (
        <IntegrationRow
            ...
            name="WhatsApp"
            description={isConnected ? t('integ.whatsapp_connected') : t('integ.whatsapp_desc')}
        ...
        >
            {!isConnected && !qrDataUrl && (
                <button onClick={handleConnect} disabled={connecting} className="px-4 py-2 rounded-lg text-[13px] font-medium text-white disabled:opacity-50" style={{ background: '#25D366' }}>
                    {connecting ? t('integ.whatsapp_connecting') : t('integ.whatsapp_connect')}
                </button>
            )}
            {qrDataUrl && !isConnected && (
                <div className="flex flex-col items-center gap-3 py-2">
                    <p className="text-[12px] font-medium" style={{ color: 'var(--text-secondary)' }}>{t('integ.whatsapp_scan')}</p>
                    ...
                    <p className="text-[11px] animate-pulse" style={{ color: 'var(--text-muted)' }}>{t('integ.whatsapp_waiting')}</p>
                </div>
            )}
